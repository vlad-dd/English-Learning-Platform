...
import { IEnglishLevel, IPreviousAnswer, ISubmittedAnswer, ISubmittedAnswers } from "../types";
...
export const useEnglishLevelWidget = () => {
    ...
    const generateLevelFeedback = (correctPoints: number) => {
        const { label } = ENGLISH_LEVELS.find((level) => correctPoints >= level.min && correctPoints <= level.max) as IEnglishLevel;
